...
CLUSTER_EPS = 0.07                      # DBSCAN neighborhood radius (m)
...
SUBPLANE_DIST_THRESHOLD = 0.02          # RANSAC inlier distance for per-cluster planes (m)
...
SAM_UNDER_SEGMENT_RATIO = 0.15         # Increased! If mask area < 35% of geom bbox, it's a tape sliver.
BBOX_PAD_RATIO = 0.40                   # Massive context for SAM
...
class SAM2Segmenter(BaseSegmenter):
    ...
    def predict(self, rgb_image: np.ndarray, box_prompt: Optional[list] = None, point_prompts: Optional[list] = None):
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            self.predictor.set_image(rgb_image)
            
            # Handle empty points safely so we can use Box-Only prompts
            if point_prompts is not None and len(point_prompts) > 0:
                pts = np.array(point_prompts)
                lbls = np.ones(len(point_prompts), dtype=np.int32)
            else:
                pts = None
                lbls = None
...
class Hybrid3DLocator:
    ...
    def _build_pick_target(self, inlier_u, inlier_v, img_shape) -> Optional[PickTarget]:
        ...
        erode_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (11, 11)) # Relaxed from 15 to prevent small box destruction
        ...
        # Minimum 100px padding so SAM isn't blinded by tight crops
        pad_w = max(int(bw * self.pad_ratio), 100)
        pad_h = max(int(bh * self.pad_ratio), 100)
...
def run_sam_on_target(segmenter: SAM2Segmenter, img_bgr, target: PickTarget, img_w, img_h):
    """Runs SAM 2 on the padded crop around the target, with an area-based
    fallback from point-only to box+point prompting."""
    ...
    # 1. Try point-only prompt
    local_mask, _ = segmenter.predict(cropped_rgb, box_prompt=None, point_prompts=local_points)

    sam_area = np.sum(local_mask)
    geom_area = w * h
    ratio = sam_area / geom_area if geom_area else 0
    
    # 2. Check if SAM latched onto a sub-feature (tape, logo, sliver)
    if ratio < SAM_UNDER_SEGMENT_RATIO:
        print(f"    -> SAM point-only under-segmented (ratio={ratio:.2f}); retrying with BOX ONLY prompt.")
        
        # Relax the Box Prompt by 25 pixels
        pm = 25
        local_box = [
            max(0, (x - x1) - pm), 
            max(0, (y - y1) - pm), 
            min(cropped_rgb.shape[1], (x + w - x1) + pm), 
            min(cropped_rgb.shape[0], (y + h - y1) + pm)
        ]
        
        # CRITICAL: We DROP the point_prompt here. If the point was on the tape, 
        # including it with the box will just force SAM to re-segment the tape!
        local_mask, _ = segmenter.predict(cropped_rgb, box_prompt=local_box, point_prompts=None)
    else:
